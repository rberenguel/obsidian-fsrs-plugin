...
function i(t, e, a) {
	return new Date(
		a
			? h(t).getTime() + e * 24 * 60 * 60 * 1e3
			: h(t).getTime() + e * 60 * 1e3,
	);
}
function s(t, e, a) {
...
function _(t, e, a) {
	let i = 1;
	for (const e of f)
		i += e.factor * Math.max(Math.min(t, e.end) - e.start, 0);
	t = Math.min(t, a);
...
class p {
	...
	constructor(t, a) {
		(this.last_review = t.last_review || t.due),
			...
			(t.elapsed_days =
				t.state === e.New ? 0 : a.diff(t.last_review, "days")),
			(t.last_review = a),
			(t.reps += 1),
...
class $ {
	...
	next_difficulty(t, e) {
		...
		return this.constrain_difficulty(
			this.mean_reversion(this.param.w[4], a),
		);
	}
		...
		switch (s.state) {
			...
			case e.Relearning:
				(l = 0),
					...
					(n = Math.max(
						this.next_interval(r.easy.stability, o),
						d + 1,
					)),
					r.schedule(a, l, d, n);
				break;
		...
		switch (n.state) {
			...
			case e.Review:
				(d = n.review),
					...
					(o =
						r.lapses -
						(n.rating === a.Again && n.state === e.Review ? 1 : 0));
				break;
		}
		const h = {
			...r,
