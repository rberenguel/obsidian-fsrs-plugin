...
export class CalendarView extends ItemView {
	...
	public async redraw(): Promise<void> {
		// If a redraw is already happening, ignore this call.
		if (this.isRedrawing) {
			return;
		}
	...
	> {
		...
		for (const noteFile of quizNotes) {
			try {
				...
				const yamlEndOffset =
					fileCache?.frontmatterPosition?.end?.offset;
				if (
					yamlEndOffset &&
					yamlEndOffset > 0 &&
				...
				const { fsrsData, identifiedClozes } =
					this.plugin.parseNoteContent(bodyContentOnly);
				...
				if (identifiedClozes.length > 0) {
					...
					for (const cloze of identifiedClozes) {
						const card: Card =
							...
							(createEmptyCard(new Date()) as Card);
						processCard(card);
					}
				} else {
					if (
				...
				console.error(
					`FSRS Calendar: Error processing note ${noteFile.path}`,
					e,
				);
			}
		}
